...
public class ZuppySecure {
	...
	public KeyPair generateKeyPair() throws NoSuchAlgorithmException {
		...
		publicKey = new String(Base64.getEncoder().encode(this.keyPair.getPublic().getEncoded()));
		return keyPair;
	}
	/*Hash Algorithms*/
	...
	public String sign(Object data, PrivateKey privateKey) throws NoSuchAlgorithmException, InvalidKeyException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException, SignatureException {
		byte[] bytes = ((String) data).getBytes();
		sign.initSign(privateKey);
		sign.update(bytes);
		byte[] signed = sign.sign();
		// encrypt signature
		String encrypted = this.encrypt(signed,"signed");
		return encrypted;
	}
	/*Verify Key*/
	public boolean verify(Object data, String signature, PublicKey publicKey) throws SignatureException, InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException {
		byte[] dataBytes = ((String) data).getBytes();
		// decrypt signature
		byte[] bytes = (byte[]) this.decrypt(signature, "signed", false);
		// start verification
